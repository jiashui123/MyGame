...
namespace MidiPlayerTK
{
    ...
    public class MidiPlayer : MonoBehaviour
    {
        ...
        /// </summary>
        /// <param name="note"></param>
        public void MPTK_PlayNote_Modify(MidiNote note)
        {
            try
            {
                // Search sample associated to the preset, midi note and velocity
                ...
                //if (smpl != null)
                {
                    ...
                    foreach (ImSample smpl in samples)
                    {
                        ...
                        if (clip != null && clip.loadState == AudioDataLoadState.Loaded)
                        {
                            ...
                            // Play note
                            StartCoroutine(PlayNote_Modifiy(audioSelected, note.Drum, smpl, note, timeToRelease));
        ...
        /// <param name="note"></param>
        /// <returns></returns>
        protected IEnumerator PlayNote_Modifiy(AudioSource audio, bool drum, ImSample sample, MidiNote note, float vTransitionTime)
        {
            if (note.Delay > 0f)
            {
                float endDelay = Time.realtimeSinceStartup + note.Delay / 1000f;
