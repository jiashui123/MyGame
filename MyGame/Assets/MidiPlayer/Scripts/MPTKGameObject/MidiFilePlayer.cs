...
namespace MidiPlayerTK
{
    ...
    public class MidiFilePlayer : MidiPlayer
    {
        ...
        public List<MidiNote> LoadMusicNote(byte[] midibytestoplay = null)
        {
            MidiLoad miditoplays = new MidiLoad();
            // No midi byte array, try to load from MidiFilesDN from resource
            if (midibytestoplay == null || midibytestoplay.Length == 0)
            {
                TextAsset mididata = Resources.Load<TextAsset>(Path.Combine(MidiPlayerGlobal.MidiFilesDB, "送别"));
                midibytestoplay = mididata.bytes;
            }

            miditoplays.KeepNoteOff = MPTK_KeepNoteOff;
            miditoplays.Load(midibytestoplay);
            //miditoplays.ChangeSpeed(MPTK_Speed);
            //miditoplays.ChangeQuantization(MPTK_Quantization);
            return miditoplays.ReadMidiEvents(miditoplays.MidiSorted[miditoplays.MidiSorted.Count - 1].Event.AbsoluteTime * miditoplays.PulseLengthMs*2);
        }
